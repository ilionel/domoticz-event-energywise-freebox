...
SWITCH_INTERNET = (
    "Freebox-Server"  # (optional) Domoticz device name of Smartplug connecting Freebox-server
)
SLIPPAGE = (
    3600  # (in seconds) time slot (after o'clock) allowing sleepmode. /!\ Value need to be < 12h
)
...
def is_ready_shut(DE, rate_limit, wake_after, no_sleep):
    ...
    Args:
        rate_limit (int): dl/ul limit (ko/s). If reached, the Freebox is not ready to shut down
        ...
        if DE.Devices["Freebox - API - Freebox Player 1"]:
            res = (
                ...
                and (
                    ...
                    or DE.Devices["Freebox - API - Next Record In"].n_value > wake_after
                )
            )
